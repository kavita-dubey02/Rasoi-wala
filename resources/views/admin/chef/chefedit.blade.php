...
<div class="container-fluid">
    <div class="card">
        ...
        <div class="card-body">
            <form action="{{ route('web-admin.chefupdate', $chef->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                    ...
                    @foreach(['current_building'=>'Building','current_street'=>'Street','current_city'=>'City','current_state'=>'State','current_pincode'=>'Pincode'] as $field=>$label)
                        <div class="col-md-4 mb-3">
                            ...
                            <input type="text" name="{{ $field }}" class="form-control"
                                value="{{ old($field, $chef->chefProfile->$field ?? '') }}">
                ...
                <div class="row">
                    @foreach(['aadhaar_front'=>'Aadhaar Front','aadhaar_back'=>'Aadhaar Back','pancard'=>'PAN Card','address_proof'=>'Address Proof'] as $field=>$label)
                        <div class="col-md-3 mb-3">
                            ...
                            @if($chef->chefDocuments?->$field)
                                <a href="{{ asset('storage/'.$chef->chefDocuments->$field) }}" target="_blank" class="d-block mt-1">View Current</a>
                            @endif
                            @error($field) <small class="text-danger">{{ $message }}</small> @enderror
                        </div>
                    @endforeach
                ...
                <div class="row">
                    @foreach(['account_holder_name'=>'Account Holder Name','account_number'=>'Account Number','ifsc_code'=>'IFSC Code'] as $field=>$label)
                        <div class="col-md-4 mb-3">
                            <label>{{ $label }}</label>
                            <input type="text" name="{{ $field }}" class="form-control"
                                value="{{ old($field, $chef->chefBank->$field ?? '') }}">
                            @error($field) <small class="text-danger">{{ $message }}</small> @enderror
                        </div>
                    @endforeach
                </div>
